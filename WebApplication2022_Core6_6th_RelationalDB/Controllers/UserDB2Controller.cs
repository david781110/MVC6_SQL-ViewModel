...
using Microsoft.AspNetCore.Mvc.Rendering;  // 搭配 SelectList

// 自己動手寫上命名空間 -- 「專案名稱.Models2」。
using WebApplication2022_Core6_6th_RelationalDB.Models2;   // 本範例放在 /Models2目錄下
// 以下命名空間，ADO.NET 會用到（請從 NuGet 自行安裝）
using System.Data;
using Microsoft.Data.SqlClient;


namespace WebApplication2022_Core6_6th_RelationalDB.Controllers
{
    public class UserDB2Controller : Controller
    {
        ...
        private readonly ILogger<UserDB2Controller> _logger;
        public UserDB2Controller(ILogger<UserDB2Controller> logger, MVC_UserDB2Context context)
        {  //                                                                                                ****************************（自己動手加上）
            _logger = logger;
            _db = context;    //*****************************（自己動手加上）
        ...
        ///      
        /// (2) 請自己在 UserTable2資料表加上FK（外部鍵）。
        ///      操作步驟，請參考 /Models2目錄下的三張圖片。
        ///      
        /// (3) /Models2/UserTable2.cs 裡面多了「驗證」，使用 System.ComponentModel.DataAnnotations命名空間
        ///      .NET Core產生的小類別，跟以前 .NET 4.x版MVC有點差異，記得自己檢查，尤其是「屬性」的名稱
        ///      
        /// (4) 改用 ADO.NET程式來存取 DB。連結字串必須加上 Trusted_Connection=True;Encrypt=False;TrustServerCertificate=False;
        ///      不然會爆錯 -- A connection was successfully established with the server, but then an error occurred during the login process. (provider: SSL Provider, error: 0 - 此憑證鏈結是由不受信任的授權單位發出的。)
        /// </summary>
        ...
        // *** 以前 .NET 4.x版MVC、EF的作法，在.NET Core出問題。*****************************************************************

        // *** [簡易版] 內容展示 -- 主表明細（Master Details / 主細表） ***
        //  一對多（一個科系有幾位學生？）
        public IActionResult IndexMasterDetails(int _ID = 3)  // 給 _ID變數（科系ID），一個預設值
        {   ////方法一：
            //（錯誤）  // return View(_db.DepartmentTable2s.Find(_ID));  // 以前 .NET 4.x版MVC、EF的作法，在.NET Core出問題。透過SQL Profiler觀察，沒有做JOIN。
            // 必須改寫如下：*******************************************************************
            //return View(_db.DepartmentTable2s.Include(x=>x.UserTable2s).Where(d => d.DepartmentId == _ID).FirstOrDefault()); 
            // 傳回單一筆數據。

            ////方法二：
            //（錯誤）  // return View(_db.DepartmentTable2s.Where(d => d.DepartmentId == _ID).Single());  // 務必確認 傳回值只有一個結果（不得為 空）
            // 以前的MVC、EF的作法，在.NET Core會出現問題。透過SQL Profiler觀察，沒有做JOIN。
            // 必須改寫如下：*******************************************************************
            ...
            //方法一 & 二：SQL Profiler翻譯後的成果 (兩者相同) ----
            #region
            // exec sp_executesql N'SELECT [t].[DepartmentId], [t].[DepartmentName], [t].[DepartmentYear], [u].[UserId], [u].[DepartmentId], [u].[UserApproved], [u].[UserBirthDay], [u].[UserMobilePhone], [u].[UserName], [u].[UserSex]
            // FROM(
            //    SELECT TOP(1)[d].[DepartmentId], [d].[DepartmentName], [d].[DepartmentYear]
            //    FROM[DepartmentTable2] AS[d]
            //    WHERE[d].[DepartmentId] = @___ID_0
            //) AS[t]
            // LEFT JOIN[UserTable2] AS[u] ON[t].[DepartmentId] = [u].[DepartmentId]
            // ORDER BY[t].[DepartmentId]',N'@___ID_0 int',@___ID_0=3
            #endregion
        }
        ...
        public IActionResult IndexMasterDetails2()  // 傳回多筆數據，檢視畫面請使用 List範本
        ...
        public IActionResult IndexMasterDetails2_Navigation(int _ID = 3)  // 給 _ID變數（科系ID），一個預設值
        {   // 透過 UserTable2s類別的「導覽屬性」來串連資料
            ...
            var result = from ut in _db.UserTable2s
                         where ut.DepartmentId == _ID  // 鎖定某一個科系（科系ID）
                         select new
                         {
            ...
            {   // 重點！！ UserTable2s的「導覽屬性」對應 -- 科系（Department2s）資料表
                sb.Append("<h3>科系ID -- " + endResult.DEPARTMENTs.DepartmentId + " -- " 
                                                               + endResult.DEPARTMENTs.DepartmentName + "</h3>");   // 科系ID（DepartmentId）
                //                                                            // *****************重點！！
        ...
        public IActionResult IndexMasterDetails2_NavigationVM(int _ID = 1)  // 給 _ID變數，一個預設值
        {   // 原廠（中文）說明  https://docs.microsoft.com/zh-tw/dotnet/framework/data/adonet/ef/language-reference/query-expression-syntax-examples-navigating-relationships
        ...
        public IActionResult IndexMasterDetails3()
        {   // 參考資料：使用 .include()   https://msdn.microsoft.com/en-us/library/jj574232(v=vs.113).aspx
        ...
        {    // 微軟範例，多對多。每一個 "一對多"的列表  
            ...
            foreach (var group in result)
            {
                ...
                foreach (var userInfo in group.TheUsers)
                {  // 這個科系的所有學生（集合，IEnumerable<UserTable2>）
        ...
        public IActionResult IndexMasterDetails5()
        {   // 參考資料： https://blog.miniasp.com/post/2010/10/14/LINQ-to-SQL-Query-Tips-INNER-JOIN-and-LEFT-JOIN.aspx
            // 第一種作法：
            var result = from d in _db.DepartmentTable2s
        ...
        public IActionResult IndexJOIN()
        {
        ...
        public IActionResult Index_AdoNet()
        {
            SqlConnection Conn = new SqlConnection("Server=.\\sqlexpress;Database=MVC_UserDB2;Trusted_Connection=True;Encrypt=False;TrustServerCertificate=False;MultipleActiveResultSets=true");
            //== 第一，連結資料庫。
        ...
        public IActionResult IndexJOINMasterDetails(int _ID = 4)  // 給 _ID變數，一個預設值
        {   // 參考資料：兩個 Table做 JOIN。  http://www.codingfusion.com/Post/How-to-Join-tables-and-return-result-into-view-usi      
        ...
        public IActionResult IndexVM1_AdoNet()
        {
        ...
        public IActionResult IndexVM2_AdoNet()
        {   //  使用 ViewModel，位於 Models2 目錄下的 UDmultiViewModel.cs檔 （一對多）
            SqlConnection Conn = new SqlConnection("Server=.\\sqlexpress;Database=MVC_UserDB2;Trusted_Connection=True;Encrypt=False;TrustServerCertificate=False;MultipleActiveResultSets=true");
            //== 第一，連結資料庫。
            Conn.Open();   //---- 開啟連結。這時候才連結DB
        ...
        public IActionResult Details(int? _ID=2)    // 網址 http://xxxxxx/UserDB2/Details?_ID=2 
        {
            if (_ID == null)
            {   // 沒有輸入文章編號（ID），就會報錯 - Bad Request
                return Content("沒有輸入內容");
            }
        ...
        public IActionResult Details2(int? _ID=2)    // 網址 http://xxxxxx/UserDB2/Details2?_ID=2 
        {
            if (_ID == null)
            {   // 沒有輸入文章編號（ID），就會報錯 - Bad Request
                return Content("沒有輸入內容");
        ...
        //=============================================
        public IActionResult Edit(int? _ID=2)    // 網址 http://xxxxxx/UserDB2/Edit?ID=2
        {
            if (_ID == null)
            {   // 沒有輸入文章編號（ID），就會報錯 - Bad Request
                return Content("沒有輸入內容");
            ...
            // 需要搭配 Microsoft.AspNetCore.Mvc.Rendering命名空間
        ...
        //public IActionResult Edit([Bind(Include = "UserId, UserName, UserSex, UserBirthDay, UserMobilePhone, UserApproved, DepartmentId")]UserTable2 _userTable)
        public IActionResult Edit(UserTable2 _userTable)
        {
            if (_userTable == null)
